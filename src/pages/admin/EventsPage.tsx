...
const EventsPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      const { data } = await supabase.from("events_content").select("*").order("content_key");
  ...
  const saveItem = async (item: ContentItem) => {
    setSaving(item.id);
    const { error } = await supabase.from("events_content").update({ content_value: item.content_value }).eq("id", item.id);
    setSaving(null);
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      if (imageItem) {
        await supabase.from("events_content").update({ content_value: data.publicUrl }).eq("id", imageItem.id);
        setContent((prev) => prev.map((c) => c.id === imageItem.id ? { ...c, content_value: data.publicUrl } : c));
      } else {
        const { data: inserted } = await supabase.from("events_content").insert({ content_key: "image_url", content_value: data.publicUrl }).select().single();
  ...
  const handleClearImage = async () => {
    ...
    const path = getStoragePath(imageItem.content_value);
    if (path) await supabase.storage.from("site-images").remove([path]);
    await supabase.from("events_content").update({ content_value: "" }).eq("id", imageItem.id);
  ...
  return (
    <div>
      <h1 className="text-2xl font-display font-bold text-foreground mb-6">Events Content</h1>
      <Card>
        <CardHeader><CardTitle>Events & Weddings</CardTitle></CardHeader>
          ...
          <div>
            ...
            {imageItem?.content_value ? (
              <div className="relative mt-2 w-full max-w-sm">
                <img src={imageItem.content_value} alt="Events" className="w-full h-40 object-cover rounded-md" />
